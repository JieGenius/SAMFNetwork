...
class PolarTransformation(nn.Module):
    def __init__(self, input_size: int, mode: str = "bilinear", max_radius=None, border_type="replicate"):
        '''
        将直角坐标转换为极坐标
        :param input_size:
        :param mode:
        ...
        map_x = map_x / (self.input_size - 1) * 2 - 1  # 这儿是因为  F.grid_sample 认为 原图中,左上角为（-1，-1）,右下角为(1,1)， 因此对其进行归一化
        ...
        *b, c, h, w = x.shape
        ...
        if len(x.shape) == 3:
            return F.grid_sample(
                x.unsqueeze(0), map, mode=self.mode, padding_mode="zeros", align_corners=True).squeeze(0)
        else:
            return F.grid_sample(x, map, mode=self.mode, padding_mode="zeros", align_corners=True)
